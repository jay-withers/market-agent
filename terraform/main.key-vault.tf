...
# (DEEPSEEK-API-KEY, and per pot ALPACA-API-KEY-<POT>, ALPACA-SECRET-KEY-<POT>
# and DEEPSEEK-API-KEY-<POT>, RESEND-API-KEY, ... — see Set-KeyVaultSecrets.ps1
# for the full list) are set with `az keyvault secret set`, so they never reach
# Terraform source or state. Each pot is a separate paper account with its own
# credential pair and its own DeepSeek key — see accounts.py.
