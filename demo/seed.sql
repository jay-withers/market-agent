...
--
-- Seeds both accounts (static-100, dynamic-500) rather than one: the
-- dashboard's account toggle and Compare view need real data on each side to
-- preview meaningfully, and dynamic-500 is deliberately given a slower
-- trajectory (see the `rate` column below) so the Compare view's "which is
-- ahead" line has something to say rather than two identical lines.
DO $$ BEGIN
...
UPDATE portfolio
   SET base_currency = 'USD', initial_cash_usd = 100000,
       cash_usd = CASE name WHEN 'static-100' THEN 97350 ELSE 98100 END,
       equity_usd = CASE name WHEN 'static-100' THEN 102480 ELSE 101690 END,
 ...
 WHERE name IN ('static-100', 'dynamic-500');

-- Shared reference data, not portfolio-scoped: both accounts price the same
-- tickers against the same market.
...
SELECT p.id,v.ticker,v.quantity,v.average,v.market_value,now()-interval '35 days',now()
  ...
  ) v(ticker,quantity,average,market_value) WHERE p.name IN ('static-100','dynamic-500');
...
SELECT p.id,d+time '06:00',d+time '06:04','succeeded','schedule',false,'demo',
 ...
 WHERE p.name IN ('static-100','dynamic-500');
...
-- dynamic-500 grows at 70% of static-100's rate — an arbitrary but fixed
-- divergence, purely so the Compare view has two visibly different lines
-- rather than two identical ones drawn on top of each other.
...
SELECT p.id,d::date,round((100000-n*30*g.rate)::numeric,4),
  ...
  FROM portfolio p
  JOIN (VALUES ('static-100',1.0),('dynamic-500',0.7)) g(name,rate) ON g.name = p.name
 CROSS JOIN LATERAL generate_series(current_date-89,current_date-1,'1 day') d
 CROSS JOIN LATERAL (SELECT (d::date-(current_date-89))::int n) x;
...
-- One combined row per day, covering both accounts — matching the real
...
SELECT as_of,
       'MarketAgent '||as_of||': static-100 $'||
         max(total_value_usd) FILTER (WHERE name='static-100')||
         ' vs dynamic-500 $'||max(total_value_usd) FILTER (WHERE name='dynamic-500'),
       'Demo daily summary for the local dashboard, covering both accounts.',
       '<p>Demo daily summary, covering both accounts.</p>',
...
VALUES (current_date-8,current_date-2,'MarketAgent demo weekly review',
        'Both accounts gained steadily in this demo window, with static-100 ahead of '
        'dynamic-500. Position limits constrained the largest recommendations on both '
