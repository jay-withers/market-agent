...
def _configure_logging() -> None:
    ...
    `marketagent` logger to Application Insights when a connection string is
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="marketagent")
    ...
    if args.command == "agent":
        ...
        try:
            ...
            logging.getLogger("marketagent").error("agent run failed: %s", exc)
            return 1
        finally:
    ...
    if args.command == "summary":
        ...
        try:
            ...
            logging.getLogger("marketagent").exception("summary failed: %s", exc)
            return 1
        finally:
            telemetry.flush()
        return 0
    ...
    if args.command == "weekly":
        ...
        try:
            ...
            logging.getLogger("marketagent").exception("weekly review failed: %s", exc)
            return 1
        finally:
            telemetry.flush()
        return 0
