...
the tests and a bare `marketagent agent` offline and free of the SDK's start-up
cost.
...
- **Only the `marketagent` logger is exported.** The distro attaches its
...
def configure(role: str) -> bool:
    ...
    configure_azure_monitor(
        ...
        logger_name="marketagent",
        enable_live_metrics=False,
        enable_performance_counters=False,
        resource=Resource.create(
            {
                "service.name": f"marketagent-{role}",
