...
import marketagent.jobs
from marketagent import cli


def _stub_job(monkeypatch, name: str, fake) -> None:
    """Replace a job module for the duration of a test.

    Patching the attribute on `marketagent.jobs`, not `sys.modules`: the CLI
    ...
    """
    monkeypatch.setattr(marketagent.jobs, name, fake, raising=False)
    monkeypatch.setitem(__import__("sys").modules, f"marketagent.jobs.{name}", fake)
...
class _FakeJob:
    """Stands in for the `marketagent.jobs.agent` module."""
...
class _FakeSummary:
    """Stands in for the `marketagent.jobs.summary` and `.weekly` modules.
