...
from marketagent.benchmarks import CASH_SYMBOL
from marketagent.jobs import weekly
from marketagent.jobs.weekly import (
    _facts_table,
    _prompt,
    _proposals_table,
    _subject,
...
def _metrics(**overrides) -> dict:
    defaults = dict(
        ...
        constraints=[
            ...
            {"binding": "max_trade_usd", "approved": True, "decisions": 3},
        ],
        trades=[
            {"status": "filled", "trades": 9, "simulated": 0, "notional_usd": D("270.0000")},
            {"status": "submitted", "trades": 3, "simulated": 0, "notional_usd": D("90.0000")},
        ],
        tickers=[
            {
                "ticker": "NVDA",
        ...
        valuation={
            "as_of": END,
            "total_value_usd": D("507.4000"),
            "cash_usd": D("330.0000"),
            "positions_value_usd": D("177.4000"),
            "pnl_usd": D("7.4000"),
            "pnl_pct": D("1.4800"),
        },
        opening_total_usd=D("502.0000"),
        benchmarks=[
            {"symbol": "SPY", "value_usd": D("512.0000"), "opening_usd": D("508.0000")},
            {"symbol": CASH_SYMBOL, "value_usd": D("501.1000"), "opening_usd": D("501.0000")},
        ],
...
def test_a_first_week_reports_no_comparison_rather_than_a_change_of_zero():
    assert _week_change(_metrics(opening_total_usd=None)) == (None, None)
    assert "no valuation before this week to compare against" in _table(opening_total_usd=None)
...
def test_a_benchmark_with_no_earlier_value_shows_no_change_rather_than_a_wrong_one():
    table = _table(benchmarks=[{"symbol": "SPY", "value_usd": D("512.0000"), "opening_usd": None}])
...
def test_the_limits_in_force_are_stated_with_the_constraints_that_bound():
    ...
    assert "| Largest single trade | $50 | `RISK_MAX_TRADE_USD` |" in table
...
def test_a_simulated_trade_is_reported_as_having_happened():
    ...
    table = _table(
        trades=[{"status": "simulated", "trades": 3, "simulated": 3, "notional_usd": D("90.0000")}]
    )
